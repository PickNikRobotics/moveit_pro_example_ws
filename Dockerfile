...
# arch also needs the other arch's object to verify. Humble builds fetch them
# too and never use them (the install step skips itself on Humble).
...
# Install nav2_mppi_controller 1.3.13 from PickNik's release bucket (Jazzy only).
#
# The Jazzy base image resolves every ROS package from a dated snapshot of that
# archive (snapshots.ros.org, see moveit_pro's Dockerfile), which still carries
# nav2 1.3.12. hangar_sim's Jazzy parameters set the MPPI `open_loop` key
# (moveit_pro issue #21202), which upstream added in nav2 1.3.13
# (ros-navigation/navigation2#6336). 1.3.13 has so far only reached the
# ros2-testing apt repository, which keeps only its newest build of each version
# and rebuilds without notice, so nothing there can be pinned durably. The exact
# debs in the nav2-mppi-debs stage above were copied from ros2-testing
# (2026-09-03 builds) to
...
#
# v9.4 still builds against Humble (the default MOVEIT_ROS_DISTRO above), whose
# nav2 1.1.20 predates `open_loop` entirely. The Humble nav2_params.yaml
# therefore never sets the key, and there is nothing to install for it: the RUN
# reads ROS_DISTRO from the sourced overlay (the image does not export it) and
# skips itself on Humble. The lag in #21202 was only observed on Jazzy; Humble
# was not measured.
#
# Remove this block and the nav2-mppi-debs stage once the Jazzy snapshot carries
# nav2 >= 1.3.13 (the notice below says when).
...
RUN --mount=type=cache,target=/var/cache/apt,sharing=locked \
    ...
    case "${ROS_DISTRO}" in \
      jazzy) \
        ...
        rm -f "${ldd_out}" ;; \
      humble) \
        echo "NOTE: skipping the nav2_mppi_controller install." \

...
TEST(GenerateRasterPath, RegressionNoTwoWaypointsCollapseOntoEachOther)
{
  // GIVEN a region whose extents sit a hair past a multiple of the spacing, so clamping the last
  // sample to the edge would place it a fraction of a millimeter from its predecessor. Cartesian
  // planning rejects such a pair ("closer than 2 * blending_radius to neighbors") and the whole
  // sweep fails -- observed live in lab sim on a hand-drawn region.
  const auto region = identityRegion();
  const lab_sim_behaviors::CoveragePathParams params{
    .line_spacing = 0.03, .point_spacing = 0.02, .standoff = 0.04, .margin = 0.0
  };
  const double x_extent = 0.30 + 1e-4;  // 15 * 0.02 plus a sliver
  const double y_extent = 0.12 + 1e-4;  //  4 * 0.03 plus a sliver

  // WHEN generating the raster
  const auto path = lab_sim_behaviors::generateRasterPath(region, { x_extent, y_extent, 0.05 }, params);

  // THEN consecutive waypoints stay at least half a spacing apart in the direction they step
  ASSERT_THAT(path, SizeIs(Gt(1)));
  const double min_separation = 0.5 * std::min(params.point_spacing, params.line_spacing);
  for (std::size_t i = 1; i < path.size(); ++i)
  {
    ...
    EXPECT_GE(std::hypot(dx, dy), min_separation)
        << "waypoints " << (i - 1) << " and " << i << " collapse onto each other";
  }

  // AND both far edges are still covered (region center is world (1, 2))
  double max_x = path.front().pose.position.x;
  double max_y = path.front().pose.position.y;
  for (const auto& pose : path)
  {
    max_x = std::max(max_x, pose.pose.position.x);
    max_y = std::max(max_y, pose.pose.position.y);
  }
  EXPECT_NEAR(max_x, 1.0 + x_extent / 2.0, 1e-6);
  EXPECT_NEAR(max_y, 2.0 + y_extent / 2.0, 1e-6);
}

TEST(GenerateRasterPath, ARegionNarrowerThanOneSpacingGetsASingleCenteredPass)
{
  // GIVEN a region only 5 mm wide in Y against a 30 mm line spacing
  const auto region = identityRegion();
  const lab_sim_behaviors::CoveragePathParams params{
    .line_spacing = 0.03, .point_spacing = 0.02, .standoff = 0.04, .margin = 0.0
  };
  ...
  // WHEN generating the raster
  const auto path = lab_sim_behaviors::generateRasterPath(region, { 0.2, 0.005, 0.05 }, params);

  // THEN it is one line down the middle, not two edges 5 mm apart
  ASSERT_FALSE(path.empty());
  for (const auto& pose : path)
  {
    EXPECT_NEAR(pose.pose.position.y, 2.0, 1e-9);
  }
}

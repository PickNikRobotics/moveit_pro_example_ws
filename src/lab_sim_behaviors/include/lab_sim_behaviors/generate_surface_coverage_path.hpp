 ...
 *
 * Samples along each axis are spread evenly between its two edges, at a step no larger than the
 * requested spacing, so both far edges are covered and no two consecutive waypoints collapse onto
 * each other (Cartesian planning rejects a pair that close). An axis narrower than one spacing gets
 * a single centered pass.
 *

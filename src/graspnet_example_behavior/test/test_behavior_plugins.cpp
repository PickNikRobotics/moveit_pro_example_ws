...
TEST(BehaviorTests, test_load_behavior_plugins)
{
  ...
  {
    auto plugin_instance = class_loader.createUniqueInstance("graspnet_example_behavior::GraspnetExampleBehaviorBehaviorsLoader");
  ...
  EXPECT_NO_THROW(
    (void)factory.instantiateTreeNode("test_behavior_name", "GraspnetExampleBehavior", BT::NodeConfiguration()));
}

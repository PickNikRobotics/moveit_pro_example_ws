 ...
 * @brief TODO(...)
 */
class GraspnetExampleBehavior : public moveit_studio::behaviors::SharedResourcesNode<BT::SyncActionNode>
{
public:
  /**
   * @brief Constructor for the graspnet_example_behavior behavior.
   ...
   */
  GraspnetExampleBehavior(const std::string& name, const BT::NodeConfiguration& config, const std::shared_ptr<moveit_studio::behaviors::BehaviorContext>& shared_resources);

  /**
   * @brief Implementation of the required providedPorts() function for the graspnet_example_behavior Behavior.
   ...
   * @return graspnet_example_behavior does not use expose any ports, so this function returns an empty list.
   */
  ...
  /**
   * @brief Implementation of BT::SyncActionNode::tick() for GraspnetExampleBehavior.

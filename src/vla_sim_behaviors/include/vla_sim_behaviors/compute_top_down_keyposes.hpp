...
/**
 * @brief Cost of reaching @p keyposes in order from @p seed_positions, or nullopt if @p solve
 * fails on any of them.
 *
 * The cost is the joint distance to the first keypose, which is where the arm arrives and so
 * the motion this segment actually spends. Solving the rest is what makes the score honest: a
 * yaw the arm can approach but not descend to is unusable, and rejecting it here lets another
 * cube symmetry win instead of failing the segment.
 *
 * @p solve carries its own state, so successive calls warm-start from the previous solution the

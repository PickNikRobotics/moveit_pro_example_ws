...
inline constexpr auto kDescriptionWaitForEpisodeStart = R"(
                ...
                <p>
                    <code>RecordEpisode</code> returns once the recorder process is spawned, and
                    the episode opens seconds later when the Trainer publishes its start marker.
                    Conversion keeps only motion after that marker, so tick this between
                    <code>RecordEpisode</code> and the demonstrated motion.
                </p>
                <p>
                    Polling the session state catches the transition whenever it lands. Pair it
                    with a short <code>WaitForDuration</code> to keep the arm still across the few
                    frames straddling the marker; the converter's idle trimming removes them.
                </p>
                <p>

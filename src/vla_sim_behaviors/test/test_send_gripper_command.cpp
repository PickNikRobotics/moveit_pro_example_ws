...
public:
   ...
   *
   * Declare one directly after the server it protects, so it runs first. A bare call at the
   * end of the test is skipped by a failing ASSERT, which is the path that aborts the binary.
   */
  class SpinGuard
  {
  public:
    explicit SpinGuard(SendGripperCommandTest& test) : test_(test)
    {
    }
...
TEST_F(SendGripperCommandTest, SucceedsWithoutWaitingForAGoalThatNeverFinishes)
{
  ...
  SpinGuard guard(*this);  // after `server`, so the executor stops first; see SpinGuard.

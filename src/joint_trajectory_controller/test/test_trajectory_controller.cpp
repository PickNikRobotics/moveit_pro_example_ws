...
TEST_P(TrajectoryControllerTestParameterized, configure_state_ignores_commands)
{
  ...
  traj_controller_->get_node()->set_parameter(rclcpp::Parameter("allow_nonzero_velocity_at_trajectory_end", true));
  ...
  // *INDENT-OFF*
  std::vector<std::vector<double>> points{ { { 3.3, 4.4, 5.5 } }, { { 7.7, 8.8, 9.9 } }, { { 10.10, 11.11, 12.12 } } };
  std::vector<std::vector<double>> points_velocities{ { { 0.01, 0.01, 0.01 } },
                                                      { { 0.05, 0.05, 0.05 } },
                                                      { { 0.06, 0.06, 0.06 } } };
  // *INDENT-ON*
  ...
  traj_controller_->update(rclcpp::Time(static_cast<uint64_t>(0.5 * 1e9)), rclcpp::Duration::from_seconds(0.5));
...
TEST_P(TrajectoryControllerTestParameterized, state_topic_legacy_consistency)
{
  ...
  EXPECT_TRUE(state->desired.accelerations.empty() || state->desired.accelerations == INITIAL_EFF_JOINTS);
...
TEST_P(TrajectoryControllerTestParameterized, state_topic_consistency)
{
  ...
  EXPECT_TRUE(state->reference.accelerations.empty() || state->reference.accelerations == INITIAL_EFF_JOINTS);
  ...
  if (std::find(command_interface_types_.begin(), command_interface_types_.end(), "position") ==
  ...
  if (std::find(command_interface_types_.begin(), command_interface_types_.end(), "effort") ==
      command_interface_types_.end())
  {
...
TEST_P(TrajectoryControllerTestParameterized, update_dynamic_tolerances)
{
  ...
  std::vector<rclcpp::Parameter> new_tolerances{ rclcpp::Parameter("constraints.goal_time", 1.0),
...
TEST_P(TrajectoryControllerTestParameterized, position_error_not_angle_wraparound)
{
  ...
  builtin_interfaces::msg::Duration time_from_start{ rclcpp::Duration(FIRST_POINT_TIME) };
  // *INDENT-OFF*
  std::vector<std::vector<double>> points{ { { 3.3, 4.4, 6.6 } }, { { 7.7, 8.8, 9.9 } }, { { 10.10, 11.11, 12.12 } } };
  // *INDENT-ON*
  ...
  if (traj_controller_->has_velocity_command_interface())
  {
    ...
    if (traj_controller_->use_closed_loop_pid_adapter())
    {
      ...
      EXPECT_NEAR(k_p * (state_reference.positions[0] - INITIAL_POS_JOINTS[0]), joint_vel_[0], k_p * COMMON_THRESHOLD);
      EXPECT_NEAR(k_p * (state_reference.positions[1] - INITIAL_POS_JOINTS[1]), joint_vel_[1], k_p * COMMON_THRESHOLD);
      EXPECT_NEAR(k_p * (state_reference.positions[2] - INITIAL_POS_JOINTS[2]), joint_vel_[2], k_p * COMMON_THRESHOLD);
    }
    else
    {
  ...
  if (traj_controller_->has_effort_command_interface())
  {
    ...
    // we expect u = k_p * (s_d-s) for positions
    EXPECT_NEAR(k_p * (state_reference.positions[0] - INITIAL_POS_JOINTS[0]), joint_eff_[0], k_p * COMMON_THRESHOLD);
    EXPECT_NEAR(k_p * (state_reference.positions[1] - INITIAL_POS_JOINTS[1]), joint_eff_[1], k_p * COMMON_THRESHOLD);
    EXPECT_NEAR(k_p * (state_reference.positions[2] - INITIAL_POS_JOINTS[2]), joint_eff_[2], k_p * COMMON_THRESHOLD);
  }
...
TEST_P(TrajectoryControllerTestParameterized, position_error_angle_wraparound)
{
  ...
  EXPECT_NEAR(state_error.positions[0], state_reference.positions[0] - INITIAL_POS_JOINTS[0], EPS);
  EXPECT_NEAR(state_error.positions[1], state_reference.positions[1] - INITIAL_POS_JOINTS[1], EPS);
  EXPECT_NEAR(state_error.positions[2], state_reference.positions[2] - INITIAL_POS_JOINTS[2] - 2 * M_PI, EPS);
  ...
  if (traj_controller_->has_velocity_command_interface())
  {
    ...
    if (traj_controller_->use_closed_loop_pid_adapter())
    {
      ...
      EXPECT_NEAR(k_p * (state_reference.positions[2] - INITIAL_POS_JOINTS[2] - 2 * M_PI), joint_vel_[2],
  ...
  if (traj_controller_->has_effort_command_interface())
  {
    ...
    EXPECT_NEAR(k_p * (state_reference.positions[2] - INITIAL_POS_JOINTS[2] - 2 * M_PI), joint_eff_[2],
                k_p * COMMON_THRESHOLD);
  }
...
TEST_P(TrajectoryControllerTestParameterized, accept_correct_cmd_timeout)
{
  ...
  SetUpAndActivateTrajectoryController(executor, { cmd_timeout_parameter, goal_time_parameter }, false);
...
{
  rclcpp::Parameter state_publish_rate_param("state_publish_rate", static_cast<double>(target_msg_count));
  rclcpp::executors::SingleThreadedExecutor executor;
...
TEST_P(TrajectoryControllerTestParameterized, use_closed_loop_pid)
{
  ...
  if ((traj_controller_->has_velocity_command_interface() && !traj_controller_->has_position_command_interface() &&
       !traj_controller_->has_effort_command_interface() && !traj_controller_->has_acceleration_command_interface() &&
...
TEST_P(TrajectoryControllerTestParameterized, velocity_error)
{
  ...
  builtin_interfaces::msg::Duration time_from_start{ rclcpp::Duration(FIRST_POINT_TIME) };
  // *INDENT-OFF*
  std::vector<std::vector<double>> points_positions{ { { 3.3, 4.4, 6.6 } },
                                                     { { 7.7, 8.8, 9.9 } },
                                                     { { 10.10, 11.11, 12.12 } } };
  std::vector<std::vector<double>> points_velocities{ { { 0.1, 0.1, 0.1 } },
                                                      { { 0.2, 0.2, 0.2 } },
                                                      { { 0.3, 0.3, 0.3 } } };
  // *INDENT-ON*
  ...
  if (traj_controller_->use_closed_loop_pid_adapter()  // always needed for PID controller
      || (traj_controller_->has_velocity_state_interface() && traj_controller_->has_velocity_command_interface()))
  {
...
TEST_P(TrajectoryControllerTestParameterized, test_jumbled_joint_order)
{
  ...
  double dt = 0.25;
  {
    ...
    const std::vector<std::string> jumbled_joint_names{ joint_names_[jumble_map[0]], joint_names_[jumble_map[1]],
                                                        joint_names_[jumble_map[2]] };
    ...
    {
      traj_msg.points[0].velocities[dof] = (traj_msg.points[0].positions[dof] - joint_pos_[jumble_map[dof]]) / dt;
      traj_msg.points[0].accelerations[dof] = (traj_msg.points[0].velocities[dof] - joint_vel_[jumble_map[dof]]) / dt;
    }
...
TEST_P(TrajectoryControllerTestParameterized, test_partial_joint_list)
{
...

  {
    ...
    std::vector<std::string> partial_joint_names{ joint_names_[jumble_map[0]], joint_names_[jumble_map[1]] };
  ...
  if (traj_controller_->has_position_command_interface())
  {
    ...
    EXPECT_NEAR(initial_joint3_cmd, joint_pos_[2], COMMON_THRESHOLD) << "Joint 3 command should be current position";
  }
  ...
  if (traj_controller_->has_velocity_command_interface())
  {
    ...
    EXPECT_TRUE(is_same_sign_or_zero(traj_msg.points[0].positions[0] - initial_joint2_cmd, joint_vel_[0]));
    EXPECT_TRUE(is_same_sign_or_zero(traj_msg.points[0].positions[1] - initial_joint1_cmd, joint_vel_[1]));
    EXPECT_NEAR(0.0, joint_vel_[2], COMMON_THRESHOLD) << "Joint 3 velocity should be 0.0 since it's not in the goal";
  }
  ...
  if (traj_controller_->has_acceleration_command_interface())
  {
    ...
    // joint rotates forward
    EXPECT_TRUE(is_same_sign_or_zero(traj_msg.points[0].positions[0] - initial_joint2_cmd, joint_acc_[0]))
        << "Joint1: " << traj_msg.points[0].positions[0] - initial_joint2_cmd << " vs. " << joint_acc_[0];
    EXPECT_TRUE(is_same_sign_or_zero(traj_msg.points[0].positions[1] - initial_joint1_cmd, joint_acc_[1]))
        << "Joint2: " << traj_msg.points[0].positions[1] - initial_joint1_cmd << " vs. " << joint_acc_[1];
    EXPECT_NEAR(0.0, joint_acc_[2], COMMON_THRESHOLD) << "Joint 3 acc should be 0.0 since it's not in the goal";
  }
  ...
  if (traj_controller_->has_effort_command_interface())
  {
    ...
    // joint rotates forward
    EXPECT_TRUE(is_same_sign_or_zero(traj_msg.points[0].positions[0] - initial_joint2_cmd, joint_eff_[0]));
    EXPECT_TRUE(is_same_sign_or_zero(traj_msg.points[0].positions[1] - initial_joint1_cmd, joint_eff_[1]));
    EXPECT_NEAR(0.0, joint_eff_[2], COMMON_THRESHOLD) << "Joint 3 effort should be 0.0 since it's not in the goal";
  }
...
TEST_P(TrajectoryControllerTestParameterized, invalid_message)
{
  ...
  rclcpp::executors::SingleThreadedExecutor executor;
  SetUpAndActivateTrajectoryController(executor, { partial_joints_parameters, allow_integration_parameters });
...
TEST_P(TrajectoryControllerTestParameterized, test_trajectory_replace)
{
  ...
  auto end_time = waitAndCompareState(expected_actual, expected_desired, executor, rclcpp::Duration(delay), 0.1);

  RCLCPP_INFO(traj_controller_->get_node()->get_logger(), "Sending new trajectory");
  points_partial_new_velocities[0][0] = std::copysign(0.15, points_partial_new[0][0] - joint_state_pos_[0]);
  ...
  waitAndCompareState(expected_actual, expected_desired, executor, rclcpp::Duration(delay), 0.1, end_time);
}
...
TEST_P(TrajectoryControllerTestParameterized, test_ignore_partial_old_trajectory)
{
  ...
  RCLCPP_INFO(traj_controller_->get_node()->get_logger(), "Sending new trajectory partially in the past");
...
TEST_P(TrajectoryControllerTestParameterized, test_execute_partial_traj_in_future)
{
  ...
  RCLCPP_WARN(traj_controller_->get_node()->get_logger(),
  ...
  RCLCPP_INFO(traj_controller_->get_node()->get_logger(), "Sending new trajectory in the future");
  publish(points_delay, partial_traj, rclcpp::Clock(RCL_STEADY_TIME).now() + delay * 2, {}, partial_traj_velocities);
  ...
  waitAndCompareState(expected_actual, expected_desired, executor, rclcpp::Duration(delay * (2 + 2)), 0.1, end_time);
}
...
TEST_P(TrajectoryControllerTestParameterized, test_jump_when_state_tracking_error_updated)
{
  ...
  double trajectory_frac =
      rclcpp::Duration::from_seconds(0.01).seconds() / (time_from_start.sec + time_from_start.nanosec * 1e-9);
  ...
  // tolerance
  EXPECT_NEAR(joint_state_pos_[0] + (second_goal[0] - joint_state_pos_[0]) * trajectory_frac, joint_pos_[0], 0.1);
  ...
  // Expect backward commands at first, consider advancement of the trajectory
  EXPECT_NEAR(joint_state_pos_[0] + (first_goal[0] - joint_state_pos_[0]) * trajectory_frac, joint_pos_[0],
              COMMON_THRESHOLD);
...
TEST_P(TrajectoryControllerTestParameterized, test_no_jump_when_state_tracking_error_not_updated)
{
  ...
  EXPECT_NEAR(first_goal[0] + (second_goal[0] - first_goal[0]) * trajectory_frac, joint_pos_[0], COMMON_THRESHOLD);
  EXPECT_GT(joint_pos_[0], first_goal[0]);
  ...
  EXPECT_NEAR(second_goal[0] + (first_goal[0] - second_goal[0]) * trajectory_frac, joint_pos_[0], COMMON_THRESHOLD);
  EXPECT_LT(joint_pos_[0], second_goal[0]);
  EXPECT_GT(joint_pos_[0], first_goal[0]);
  end_time = updateControllerAsync(rclcpp::Duration::from_seconds(0.01), end_time);
  EXPECT_GT(joint_pos_[0], first_goal[0]);
...
TEST_P(TrajectoryControllerTestParameterized, test_hw_states_has_offset_first_controller_start)
{
  ...
  SetUpAndActivateTrajectoryController(executor, { is_open_loop_parameters }, true, 0., 1., false, initial_pos_cmd,
                                       initial_vel_cmd, initial_acc_cmd);
...
TEST_P(TrajectoryControllerTestParameterized, test_state_tolerances_fail)
{
  ...
  std::vector<rclcpp::Parameter> params = { rclcpp::Parameter("constraints.joint1.trajectory", state_tol),
...
TEST_P(TrajectoryControllerTestParameterized, test_goal_tolerances_fail)
{
  ...
  std::vector<rclcpp::Parameter> params = { rclcpp::Parameter("constraints.joint1.goal", goal_tol),
...
INSTANTIATE_TEST_SUITE_P(PositionTrajectoryControllers, TrajectoryControllerTestParameterized,
                         ::testing::Values(std::make_tuple(std::vector<std::string>({ "position" }),
...
INSTANTIATE_TEST_SUITE_P(PositionVelocityTrajectoryControllers, TrajectoryControllerTestParameterized,
                         ::testing::Values(std::make_tuple(std::vector<std::string>({ "position", "velocity" }),
                                                           std::vector<std::string>({ "position" })),
...
INSTANTIATE_TEST_SUITE_P(
    ...
    ::testing::Values(std::make_tuple(std::vector<std::string>({ "position", "velocity", "acceleration" }),
                                      std::vector<std::string>({ "position" })),
...
INSTANTIATE_TEST_SUITE_P(OnlyVelocityTrajectoryControllers, TrajectoryControllerTestParameterized,
                         ::testing::Values(std::make_tuple(std::vector<std::string>({ "velocity" }),
                                                           std::vector<std::string>({ "position", "velocity" })),
                                           std::make_tuple(std::vector<std::string>({ "velocity" }),
                                                           std::vector<std::string>({ "position", "velocity",
                                                                                      "acceleration" }))));

// only effort controller
INSTANTIATE_TEST_SUITE_P(OnlyEffortTrajectoryControllers, TrajectoryControllerTestParameterized,
                         ::testing::Values(std::make_tuple(std::vector<std::string>({ "effort" }),
                                                           std::vector<std::string>({ "position", "velocity" })),
                                           std::make_tuple(std::vector<std::string>({ "effort" }),
                                                           std::vector<std::string>({ "position", "velocity",
                                                                                      "acceleration" }))));

 ...
 * @brief TODO(...)
 */
class StringToInt : public BT::SyncActionNode
{
public:
  /**
  ...
  /**
   * @brief Implementation of BT::SyncActionNode::tick() for StringToInt.
   * @details This function is where the Behavior performs its work when the behavior tree is being run. Since
   * StringToInt is derived from BT::SyncActionNode, it is very important that its tick() function always

...
- **[J. Deep Troubleshooting](J_deep_troubleshooting.md)** — long-form
  root-cause investigations: GigE bandwidth bottleneck (15 Hz dual-cam
  story), ApproxTimeSync stamp-offset compensation, and brightness-based
  ZED frame filtering for separating strobed from ambient frames in
  post-processing.
...
| `bag_extract.py`        | Post-hoc: extract image frames from a bag, auto-sort by brightness (separates strobed from ambient ZED frames) | B §7 |
| `bag_panels.py`         | Post-hoc: composite panels of synced Cam1+Cam2+ZED frames for visual sync inspection | (debug only)  |

...
TEST(BehaviorTests, test_load_behavior_plugins)
{
  ...
  {
    auto plugin_instance = class_loader.createUniqueInstance("pcl_decompose_to_spheres::PclDecomposeToSpheresBehaviorsLoader");
  ...
  EXPECT_NO_THROW(
    (void)factory.instantiateTreeNode("test_behavior_name", "PclDecomposeToSpheres", BT::NodeConfiguration()));
}

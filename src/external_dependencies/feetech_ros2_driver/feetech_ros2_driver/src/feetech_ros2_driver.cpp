...
// PickNik addition. Upstream 0.2.2 never enables torque: an STS3215 powers up
// with TORQUE_ENABLE set, on_deactivate clears it, and nothing sets it again, so
// a restarted stack executes trajectories into limp servos. This makes the
// torque state follow the lifecycle: off at configure, on at activate, off at
// deactivate. Joints without a command interface are left off at activate, the
// same rule on_init applies, so a state-only bus (a leader arm) stays limp.
...
CallbackReturn FeetechHardwareInterface::on_configure(const rclcpp_lifecycle::State& /* previous_state */) {
  // all joints torque off until on_activate
  return set_torque(joint_ids_, false, "on_configure");
}
...
CallbackReturn FeetechHardwareInterface::on_activate(const rclcpp_lifecycle::State& /* previous_state */) {
  ...
  // the first write() holds the pose the arm is already in
  std::vector<uint8_t> commanded_joint_ids;
  for (uint i = 0; i < info_.joints.size(); i++) {
    if (!info_.joints[i].command_interfaces.empty()) {
      commanded_joint_ids.push_back(joint_ids_[i]);
...
CallbackReturn FeetechHardwareInterface::on_deactivate(const rclcpp_lifecycle::State& /* previous_state */) {
  ...
  return set_torque(joint_ids_, false, "on_deactivate");
}

...
class FeetechHardwareInterface : public hardware_interface::SystemInterface {
 ...
 private:
  // PickNik addition: TORQUE_ENABLE on/off for `ids` in one sync write.
  CallbackReturn set_torque(const std::vector<uint8_t>& ids, bool enable, std::string_view caller);

...
class DoObjectiveSequenceResource:
    ...
    def send_goal(self, goal):
        self.action_data.send_goal_future = self.do_objective_sequence_client.send_goal_async(goal)

    def cancel_goal(self):
        if self.action_data.goal_handle is not None:
            self.action_data.cancel_future = self.action_data.goal_handle.cancel_goal_async()
            self.action_data.cancel_future.add_done_callback(self._cancel_done_callback)

    def wait_for_cancel_response(self, timeout_sec):
        self.callback_on_future_complete(self.action_data.cancel_future, self._cancel_done_callback,
                                         timeout_sec=timeout_sec)

    def wait_for_goal_response(self, timeout_sec):
        self.callback_on_future_complete(self.action_data.send_goal_future, self._goal_response_callback,
                                         timeout_sec=timeout_sec
                                         )

    def wait_for_result(self, timeout_sec):
        self.callback_on_future_complete(self.action_data.get_result_future, self._get_result_callback,
                                         timeout_sec=timeout_sec)

    def callback_on_future_complete(self, future, callback, timeout_sec=-1):
        current_time = self.node.get_clock().now()
        while not future.done():
            if timeout_sec - (self.node.get_clock().now() - current_time).to_msg().sec == 0:
                return
    ...
    def _goal_response_callback(self, future):
        ...
        self.action_data.get_result_future.add_done_callback(self._get_result_callback)
...
def test_objective_server(
        objective_xml, do_objective_sequence_resource: DoObjectiveSequenceResource
):
    time.sleep(1)

    cancel_objectives = {"Teleoperate", "Joint Diagnostic", 'Segment Point Cloud from Clicked Point', 'Place Object',
                         'Loop Detect AprilTag', 'Grasp Pose Tuning With April Tag', 'Grasp Object from Text Prompt',
                         'Grasp Planning', '3 Waypoints Pick and Place', 'Pick And Place Example',
                         'Record and Replay Scanning Motion', 'Plan and Save Trajectory', 'Cycle Between Waypoints',
                         'Pick April Tag Labeled Object With Approval'}
    skip_objectives = {'Joint Diagnostic'}
    objective = parse_objective_xml(objective_xml)
    ...
    if not objective.id in cancel_objectives:
        ...
        assert action_data.cancel_response.return_code == 0

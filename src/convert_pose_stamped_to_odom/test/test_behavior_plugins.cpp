...
TEST(BehaviorTests, test_load_behavior_plugins)
{
  ...
  {
    auto plugin_instance = class_loader.createUniqueInstance("convert_pose_stamped_to_odom::ConvertPoseStampedToOdomBehaviorsLoader");
  ...
  EXPECT_NO_THROW(
    (void)factory.instantiateTreeNode("test_behavior_name", "ConvertPoseStampedToOdom", BT::NodeConfiguration()));
}

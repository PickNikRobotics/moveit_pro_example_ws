...
def test_a_stamp_from_the_future_is_stale() -> None:
    """Sim time rewinds on a scene reset, leaving the previous episode's stamp ahead of now.

    Reading that as a fresh reference would label the first frames of an episode with the
    setpoint the arm was chasing in the last one.
    """
    assert not is_reference_fresh(
        now=1.0, stamp=9.0, timeout=DEFAULT_REFERENCE_TIMEOUT_S
    )
    assert not is_reference_fresh(
        now=1.0, stamp=1.1, timeout=DEFAULT_REFERENCE_TIMEOUT_S
    )
    assert is_reference_fresh(now=1.0, stamp=1.0, timeout=DEFAULT_REFERENCE_TIMEOUT_S)

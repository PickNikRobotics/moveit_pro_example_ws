...
TEST(BehaviorTests, test_load_behavior_plugins)
{
  ...
  {
    auto plugin_instance = class_loader.createUniqueInstance("convert_transform_stamped_to_odom::ConvertTransformStampedToOdomBehaviorsLoader");
  ...
  EXPECT_NO_THROW(
    (void)factory.instantiateTreeNode("test_behavior_name", "ConvertTransformStampedToOdom", BT::NodeConfiguration()));
}

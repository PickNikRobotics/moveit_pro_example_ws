...
{
/**
 * @brief Run Aruco tag detection on a provided image and output the detected pose.
 */
class DetectArucoTag : public moveit_studio::behaviors::SharedResourcesNode<BT::SyncActionNode>
{
public:
  /**
   * @brief Constructor for the detect_aruco_tag behavior.
   ...
   */
  DetectArucoTag(const std::string& name, const BT::NodeConfiguration& config,
                 const std::shared_ptr<moveit_studio::behaviors::BehaviorContext>& shared_resources);

  /**
   * @brief Implementation of the required providedPorts() function for the detect_aruco_tag Behavior.
   ...
   * @return detect_aruco_tag does not expose any ports, so this function returns an empty list.
   */
  ...
  /**
   * @brief Implementation of BT::SyncActionNode::tick() for DetectArucoTag.
   * @details This function is where the Behavior performs its work when the behavior tree is being run. Since
   * DetectArucoTag is derived from BT::SyncActionNode, it is very important that its tick() function always finishes

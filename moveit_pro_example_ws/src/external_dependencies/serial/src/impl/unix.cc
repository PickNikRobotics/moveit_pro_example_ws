...
void Serial::SerialImpl::reconfigurePort()
{
  ...
  switch (baudrate_)
  {
    ...
    default:
      ...
      struct serial_struct set;

      if (-1 == ioctl(fd_, TIOCGSERIAL, &set))
      {
        THROW(IOException, errno);
      }

      // set custom divisor
      set.custom_divisor = set.baud_base / static_cast<int>(baudrate_);
      // update flags
      set.flags &= ~ASYNC_SPD_MASK;
      set.flags |= ASYNC_SPD_CUST;

      if (-1 == ioctl(fd_, TIOCSSERIAL, &set))
      {
        THROW(IOException, errno);
      }
#else
